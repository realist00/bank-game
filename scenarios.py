# app.py - Google Sheets 실시간 영구 연동 및 서식 완결본
...
# ==============================================================================
# 3. 데이터 저장소 관리 (Google Sheets 실시간 동기화 + 로컬 백업)
# ==============================================================================
...
def set_gsheet_url(url):
    with open(CONFIG_FILE, "w", encoding="utf-8") as f:
        json.dump({"url": url.strip()}, f)

def sync_from_gsheet(url):
...
def _load_data():
    g_url = get_gsheet_url()
    if g_url:
        g_data = sync_from_gsheet(g_url)
        if g_data:
            _save_local(g_data)
            return _normalize(g_data)
...
current_gsheet = get_gsheet_url()
if current_gsheet:
    st.sidebar.success("🟢 구글 시트 영구 동기화 ON")
else:
    st.sidebar.warning("⚪ 로컬 임시 저장 모드")
...
# -------------------------------------------------------------
    ...
    adm_tab1, adm_tab2, adm_tab3 = st.tabs(["🕹️ 라운드 진행 및 결산", "📈 전체 성과 종합 비교", "⚙️ 구글시트 연동 및 데이터 관리"])
    ...
    with adm_tab3:
        st.markdown("### ⚙️ 구글 시트 영구 연동 및 데이터 관리")
        
        # 1. 구글 시트 웹 앱 URL 설정
        st.markdown("#### 🔗 Google Sheets 실시간 영구 연동 설정")
        current_g_url = get_gsheet_url()
        with st.form("gsheet_url_form"):
            input_g_url = st.text_input(
                "Google Apps Script 웹 앱 URL 입력",
                value=current_g_url,
                placeholder="https://script.google.com/macros/s/.../exec"
            )
            btn_save_url = st.form_submit_button("구글 시트 연동 URL 저장")
            if btn_save_url:
                set_gsheet_url(input_g_url)
                st.success("구글 시트 연동 URL이 저장되었습니다!")
                st.rerun()
                
        if current_g_url:
            st.success("🟢 **Google Sheets 실시간 영구 동기화가 활성화되어 있습니다.**")
            c_g1, c_g2 = st.columns(2)
            with c_g1:
                if st.button("🔄 [수동] 구글 시트에서 최신 데이터 즉시 불러오기"):
                    pulled = sync_from_gsheet(current_g_url)
                    if pulled:
                        _save_local(pulled)
                        st.success("구글 시트의 최신 데이터를 완벽하게 불러왔습니다!")
                        st.rerun()
                    else:
                        st.error("구글 시트로부터 데이터를 가져오지 못했습니다. URL을 확인해 주세요.")
            with c_g2:
                if st.button("💾 [수동] 현재 데이터를 구글 시트로 강제 전송"):
                    ok = sync_to_gsheet(current_g_url, data)
                    if ok:
                        st.success("현재 모든 데이터가 구글 시트에 안전하게 백업되었습니다!")
                    else:
                        st.error("구글 시트로 데이터를 전송하지 못했습니다.")
        else:
            st.warning("⚠️ 아직 구글 시트 URL이 설정되지 않았습니다. 현재는 로컬 임시 저장소로 동작 중입니다.")
